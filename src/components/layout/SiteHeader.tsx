 ...
 *
 * Always on the dark ground, whatever section scrolls beneath it.
 */
      ...
      <Container className="flex h-16 items-center justify-between gap-6 lg:h-20">
        ...
        <nav aria-label={COPY.nav.menu} className="hidden items-center gap-8 md:flex">
          {MAIN_NAV.map((item) => (
            <Link
              key={item.href}
              href={item.href}
              className="text-fg-muted hover:text-fg after:bg-accent relative py-1 font-[family-name:var(--font-ui)] text-xs font-semibold tracking-[0.12em] uppercase transition-colors after:absolute after:-bottom-0.5 after:left-0 after:h-px after:w-0 after:transition-all after:duration-300 hover:after:w-full"
            >
              {item.label}
            </Link>
          ))}
        ...
        <div className="flex items-center gap-5">
          <a
            ...
            className="u-numeric text-fg hover:text-accent-text hidden items-center gap-2.5 text-sm transition-colors xl:flex"
          >
            <PhoneIcon />
            {SITE.contact.phone}
          </a>
